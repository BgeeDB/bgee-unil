...
const useLogic = (isExprCalls) => {
  ...
  const prepTermHierarchy = (expressionCalls) => {
    ...
    expressionCalls.forEach((exprCall) => {
      ...
      const termLabel = cellTypeId !== 'GO:0005575' ? `${anatEntityName} : ${cellTypeName}` : anatEntityName;
    ...
    function createNestedStructure(termId, depth = 0) {
      ...
      const nestedTerm = {
        id: termId,
        label: term.cellTypeId !== 'GO:0005575' ? `${term.label} : ${term.cellTypeLabel}` : term.label,
