    ...
    '@graph': [
      {
        ...
        url: 'https://bgee.org/',
        description:
          'The aim of Bgee is to help biologists to use and understand gene expression',
...
const speciesToLdJSON = ({
  ...
  if (file) {
    json.subjectOf[1].hasPart.push({
      ...fileDownloadProps(file),
      name: `${genus} ${speciesName} ${FULL_LENGTH_LABEL} experiment libraries`,
      description: `${genus} ${speciesName} ${FULL_LENGTH_LABEL} experiments/ libraries annotations and metadata.`,
      keywords: [FULL_LENGTH_LABEL, 'Single cell RNA-Seq'],
  ...
  if (file) {
    json.subjectOf[1].hasPart.push({
      ...fileDownloadProps(file),
      name: `${genus} ${speciesName} ${FULL_LENGTH_LABEL} read counts, TPM and FPKM`,
      description: `${genus} ${speciesName} ${FULL_LENGTH_LABEL} read counts, TPM (Transcript Per Million) and FPKM (Fragments Per Kilobase of transcript per Million mapped reads).`,
      keywords: [FULL_LENGTH_LABEL, 'Single cell RNA-Seq'],
      url: `${window.location.href}#proc-values-fl-scrna-seq`,
    });
  }
